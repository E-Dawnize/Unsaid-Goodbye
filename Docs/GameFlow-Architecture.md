...
GameFlow 系统采用 **MVVM + DI（依赖注入）** 架构，将剧情流程拆分为三层独立模块。Manager 负责业务逻辑，Model 持有运行时状态，View 处理表现层。模块间通过 DI 容器注入、事件驱动、以及 Unity Inspector 绑定协作。
...
│                                                                             │
│  Register:  IGameFlowManager → GameFlowManager  (Singleton)          │
...
└────────────────────────────┬─────────────────────────────────────────────┘
         ...
         ▼                    ▼                    ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────────┐
│  GameFlowModel  │  │ GameFlowManager│  │   GameFlowView    │
│   (Model)       │  │   (Manager)       │  │   (View)            │
│                 │  │                  │  │                     │
│ ScriptableObject│  │ 纯 C# 类         │  │ MonoBehaviour       │
│ INotifyProperty │  │ IGameFlowManager│  │ StrictLifecycleMono│
...
├──────────────────────────────────────────────────────────────┤
│ - _manager: IGameFlowManager      [Inject]              │
├──────────────────────────────────────────────────────────────┤
...
```
     Player/Event      GameFlowManager       GameFlowModel        GameFlowView
         │                    │                      │                    │
         │  Trigger Event     │                      │                    │
         │───────────────────►│                      │                    │
         │                    │                      │                    │
...
```
DIContainer
├── IGameFlowManager → GameFlowManager       (Singleton)
...
`GameFlowManager` 是纯 C# 类，仅依赖 `IEventCenter` 和 `GameFlowModel`。它通过 `OnPhaseChanged` / `OnPhaseComplete` 事件通知 View，不直接调用任何 Unity 场景操作。
...
游戏内交互（收集道具、解谜、进入触发区）通过 `IEventCenter` 发布事件，Manager 订阅后将事件匹配到对应 `StoryBeat`，完成进度判定。这使得：
- 互动模块与剧情模块完全解耦
...
Unity.Addressables ──► GamePlayInstaller ──► DIContainer
                              ...
                              │               │               │
                              ▼               ▼               ▼
                     GameFlowManager   GameFlowView   Bindings
                              │               │               │
                              ▼               ▼               │
...
| 文件 | 角色 | 命名空间 |
|---|---|---|
| `Assets/Gameplay/Interfaces/IGameFlowManager.cs` | Manager 接口 | `Gameplay.Interfaces` |
| `Assets/Gameplay/SceneFlow/GameFlowManager.cs` | Manager 实现 | `Gameplay.SceneFlow` |
| `Assets/Gameplay/SceneFlow/GameFlowView.cs` | View 表现层 | `Gameplay.SceneFlow` |
