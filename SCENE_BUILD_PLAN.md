...
- Phase2 推进：公园段已按新大纲拆为 `SurfaceWorld_Park`（叼项圈、安葬）和 `InnerWorld_Park`（回头看脚印/狗肉车）。
...
| `Assets/Scenes/SurfaceWorld_Park.unity` | 已存在 | `Def_DogCollar`, `Def_BuryCollar` |
| `Assets/Scenes/InnerWorld_Park.unity` | 已存在 | `Def_DogLookBack` |
| `Assets/Scenes/Old Home Bedroom.unity` | 已存在 | `Def_CrateLabel`, `Def_BedCornerSavings`, `Def_BuryOrangeCat` |
| `Assets/Scenes/InnerWorld_Bedroom_UnderBed.unity` | 已复制占位 | `Def_WallDiary` |
| `Assets/Scenes/InnerWorld_Bedroom_Mirror.unity` | 已复制占位 | `Def_BedroomMirrorHint` |
| `Assets/Scenes/Balcony 2.unity` | 已存在 | `Def_CageDoor`, `Def_CageWater`, `Def_ParrotFeather` |
| `Assets/Scenes/OutterWorld_Living Room 3.unity` | 已存在 | `Def_LivingRoomMirror`, `Def_SofaOwner`, `Def_EndingChoice` |
...
| Phase2 表世界公园 | `Scenes/SurfaceWorld_Park` | 已从公园场景复制，需 Unity 内换成表世界公园美术/微调位置 | `dialogue_phase2_intro`, `dialogue_l1_dog_encounter`, `dialogue_bury_collar` | `Def_DogCollar`, `Def_BuryCollar` |
| Phase2 里世界公园 | `Scenes/InnerWorld_Park` | 已保留回头看触发，需 Unity 内换成脚印/狗肉车版本美术 | `dialogue_dog_lookback` | `Def_DogLookBack` |
...
| Phase4A 表世界卧室/床底 | `Scenes/Old Home Bedroom` | 已拆为表世界床底段，需 Unity 内替换/微调美术 | `dialogue_phase4_bedroom_intro`, `dialogue_l3_crate_label`, `dialogue_h4_savings`, `dialogue_bury_orange_cat` | `Def_CrateLabel`, `Def_BedCornerSavings`, `Def_BuryOrangeCat` |
| Phase4B 里世界床底/日记 | `Scenes/InnerWorld_Bedroom_UnderBed` | 已复制占位场景并新增日记触发区 | `dialogue_l4_wall_diary` | `Def_WallDiary` |
| Phase4C 里世界卧室/镜子 | `Scenes/InnerWorld_Bedroom_Mirror` | 已复制占位场景并新增镜子提示触发区 | `dialogue_bedroom_mirror_hint` | `Def_BedroomMirrorHint` |
...
- 目标场景：`Scenes/SurfaceWorld_Park`
- 当前状态：已从 `InnerWorld_Park` 复制为表世界公园阶段，保留 `DogCollar_Interactable` 和 `BuryCollar_Interactable`，禁用 `DogLookBack_Interactable`。
- 备注：完成 `Def_DogCollar` 和 `Def_BuryCollar` 后进入 `Scenes/InnerWorld_Park`。
...
| 物品/触发点 | L1 项圈狗牌 | 已挂 `DogCollar_Interactable`，需 Unity 内微调 |
|  | 流浪狗 | 待场景中确认/摆放 |
|  | 安葬项圈触发点 | 已挂 `BuryCollar_Interactable`，需 Unity 内微调/替换土堆表世界版本 |
...
| 交互定义 | `Def_DogCollar` | 已挂到场景，加入 Phase2 Surface RequiredBeats |
|  | `Def_BuryCollar` | 已挂到场景，加入 Phase2 Surface RequiredBeats |
...
- 当前状态：保留原公园场景作为里世界真相段，禁用 `DogCollar_Interactable` 和 `BuryCollar_Interactable`，只保留 `DogLookBack_Interactable`。
- 备注：完成 `Def_DogLookBack` 后进入 `Scenes/OutterWorld_Living Room 2`。

| 类型 | 内容 | 资源/状态 |
| --- | --- | --- |
| 背景 | `Assets/Arts/公园/moon park.jpg` | 已找到 |
|  | `Assets/Arts/公园/moon park.psb` | 已找到 |
| 物品/触发点 | 回头看脚印/狗肉车方向 | 已挂 `DogLookBack_Interactable`，需 Unity 内微调/等待美术补脚印与狗肉车版本 |
| 对白 | `dialogue_dog_lookback` | 已有 DialogueSequence，已更新为脚印 + 狗肉车真相文本 |
| 交互定义 | `Def_DogLookBack` | 已挂到场景，加入 Phase2 Inner RequiredBeats |
| 音频 | `Assets/Arts/BGM/BGM/park-inner world.wav` | 已找到 |
...
- 当前状态：按新版大纲改为“表世界卧室/床底”阶段；已新增 `CrateLabel_Interactable`、`BedCornerSavings_Interactable`、`BuryOrangeCat_Interactable` 三个占位触发区。
- 备注：此阶段完成 L3 航空箱标签、H4 床底存钱罐、安葬橘猫后，自动进入 `Scenes/InnerWorld_Bedroom_UnderBed`。触发区是空物体占位，需在 Unity 里拖到对应美术物体上。

| 类型 | 内容 | 资源/状态 |
| --- | --- | --- |
| 背景 | `Assets/Arts/卧室/Bedroom.png` | 已找到 |
...
- 目标场景：`Scenes/InnerWorld_Bedroom_UnderBed`
- 当前状态：从 `Old Home Bedroom.unity` 复制的占位场景；已新增 `WallDiary_Interactable`。
- 备注：此阶段只要求阅读 L4 日记；读完后自动进入 `Scenes/InnerWorld_Bedroom_Mirror`。后续需要把背景换成更新后的里世界床底素材。

| 类型 | 内容 | 资源/状态 |
| --- | --- | --- |
| 背景 | `Assets/Arts/床下/moon under the bed .jpg` | 已找到，建议作为里世界床底方向 |
| 物品/触发点 | L4 墙上日记/日记碎片 | 已挂 `WallDiary_Interactable` 占位触发区，需 Unity 内微调 |
| 对白 | `dialogue_l4_wall_diary` | 已有 DialogueSequence |
| 交互定义 | `Def_WallDiary` | 已加入 `InnerBedroomUnderBed.asset` 的 RequiredBeats |
| 音频 | 待接入：日记/碎片音效 | 待确认 |

### Phase4C 里世界卧室/镜子

- 目标场景：`Scenes/InnerWorld_Bedroom_Mirror`
- 当前状态：从 `Old Home Bedroom.unity` 复制的占位场景；已新增 `BedroomMirrorHint_Interactable`。
- 备注：此阶段用于新版大纲中“镜子里好像少了点什么”的提前伏笔；完成后自动进入 `Scenes/Balcony 2`。

| 类型 | 内容 | 资源/状态 |
| --- | --- | --- |
| 背景 | `Assets/Arts/卧室/moon Bedroom.jpg` | 已找到，建议作为里世界卧室方向 |
| 物品/触发点 | 卧室镜子 | 已挂 `BedroomMirrorHint_Interactable` 占位触发区，需 Unity 内摆到镜子上 |
| 对白 | `dialogue_bedroom_mirror_hint` | 已新增 DialogueSequence |
| 交互定义 | `Def_BedroomMirrorHint` | 已新增 InteractionDef，并加入 `InnerBedroomMirror.asset` 的 RequiredBeats |
| 音频 | 待接入：镜子伏笔音效 | 待确认 |
...
3. 在 `Old Home Bedroom` 中测试 L3/H4/橘猫安葬三个交互，并微调航空箱标签、存钱罐、橘猫位置。
4. 在 `InnerWorld_Bedroom_UnderBed` 中测试 L4 日记交互，并把背景换成里世界床底版本。
5. 在 `InnerWorld_Bedroom_Mirror` 中测试卧室镜子伏笔交互，并把触发区摆到镜子上。
6. 在 `Balcony 2` 中测试鸟笼门、食槽/水、羽毛三个交互，并微调触发范围。
7. 在 `OutterWorld_Living Room 3` 中测试镜子、主人真相和结局选择，并微调触发范围。
8. 最后补 `Epilogue_Stay` / `Epilogue_Leave`；结局图暂缺时先用占位资源跑通。
...
- 2026-05-20：根据新版内容大纲重构公园段：新增 `SurfaceWorld_Park.unity` 和 `Phase2_SurfacePark.asset`；`StartPhase` 结束后进入表世界公园，完成项圈/安葬后进入 `InnerWorld_Park`，里世界公园只保留回头看触发。
- 2026-05-20：根据新版内容大纲重构卧室段：Phase4 拆为 `Old Home Bedroom`（L3/H4/安葬橘猫）、`InnerWorld_Bedroom_UnderBed`（L4 日记）、`InnerWorld_Bedroom_Mirror`（卧室镜子伏笔），并将阳台、最终镜子、结局阶段顺延。
...
- 2026-05-18：Phase4 旧家卧室改为 `Scenes/Old Home Bedroom`；当时新增床底背景、橘猫、航空箱标签、日记和四个交互触发区，后续已按新版大纲拆分。
