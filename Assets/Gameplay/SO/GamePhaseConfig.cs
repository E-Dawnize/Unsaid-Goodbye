...
namespace Gameplay.SO
{
    ...
    // public class GamePhaseConfigRuntime
    // {
    //     [Header("基础信息")]
    //     public GamePhase PhaseId;                    // 阶段ID
    //     public string DisplayName;                   // 显示名称（调试用）
    ...
    //
    //     [Header("流程控制")]
    //     public Dictionary<string ,StoryBeat> RequiredBeats;         // 本阶段需要完成的故事节拍
    //     public GamePhase DefaultNextPhase;            // 默认下一阶段
    //     public GamePhase AltNextPhase;               // 备选下一阶段（用于结局分支）
    //     public bool IsEndingBranch;                   // 是否在此阶段判定结局
    //
    ...
    //     public string TransitionSFX;                  // 转场音效ID
    //     public float TransitionDuration = 2.0f;       // 转场黑屏时长
    //     public GamePhaseConfigRuntime(GamePhaseConfig config)
    //     {
    //         PhaseId = config.PhaseId;                  
    //         DisplayName=config.DisplayName;             
    //         SceneAssetPath=config.SceneAssetPath;      
    //         //public BGMTrack BackgroundMusic;         
    //         EntryDialogueId=config.EntryDialogueId;             
    //         ExitDialogueId=config.ExitDialogueId;               
    //         foreach (var beat in config.RequiredBeats)
    //         {
    //             RequiredBeats[beat.beatID] = beat;
    //         }
    //         DefaultNextPhase=config.DefaultNextPhase;          
    //         AltNextPhase=config.AltNextPhase;              
    //         IsEndingBranch=config.IsEndingBranch;                
    //         TransitionSFX=config.TransitionSFX;               
